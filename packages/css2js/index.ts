...
*/
function isolateSelectors(styles: css.Stylesheet): Styles {
  const transformed: Styles = styles.stylesheet ? styles.stylesheet.rules?.reduce((all: Styles, rule: css.Rule | css.Media) => {
    switch (rule.type) {
      ...
      case 'media':
        ...
        const mediaStyles = m.rules ? transform({ stylesheet: { rules: m.rules } }) : {};
 ...
 */
function concatSelectors(transformed: Styles): Styles {
 ...
 * @param {css.Stylesheet} styles - the parsed CSS stylesheet
 */
function transform(styles: css.Stylesheet): Styles {
  return concatSelectors(isolateSelectors(styles))
}
 ...
 */
function csstojs(cssString: string): Styles {
